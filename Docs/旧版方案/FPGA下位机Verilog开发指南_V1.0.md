...
1. **本开发指南 V1.0 及后续变更记录**：内部接口、寄存器、USB 协议、模块边界；
...
| 内部数据字宽 | 32 bit |
| 寄存器数据宽度 | 32 bit |
| 寄存器地址宽度 | 32 bit；V1.0 只占低 16 bit |
...
├── dac/
│   ├── dac_ad5791_if.v
│   └── dac_ad3552_if.v       # 预留，不作为 V1.0 必选
├── adc/
│   ├── adc_ad4630_if.v
│   ├── adc_adc3660_if.v
│   └── adc_cycle_framer.v
...
# 3.3 统一消息流 `msg_stream`

所有已经形成“记录/帧”的数据通过该接口进入 `stream_arbiter -> data_packetizer`：
...
- `m_sof=1` 仅出现在一个消息的第一个有效 beat。
- 元数据 `source_id/msg_id/timestamp/cycle_id/flags` 只要求在 `m_sof=1 && m_valid=1` 时有效；packetizer 在首拍锁存。
- `m_keep[3:0]` 每 bit 表示 `m_data` 对应字节是否有效；非最后一拍固定 `4'b1111`。
- `m_last=1` 表示 payload 最后一拍。
- 当 `m_valid=1 && m_ready=0` 时，所有输出保持不变。
- 允许 `payload_len=0` 的 event；此时产生一个 `m_sof=m_last=1` 的空 payload token，可由 packetizer 特殊处理。
...
V1.0 `timestamp` = 对应 WMS `scan_start` 在 `sys_clk` 域被确认时的 local tick。
...
|---:|---|---|
| 0 | u16 | schema_version = 1 |
| 2 | u16 | format：0=I/Q，1=I/Q+Magnitude |
| 4 | u32 | output_rate_hz |
| 8 | u32 | point_count |
| 12 | u32 | first_point_index |
| 16 | records | 结果 |
...
数值的 fixed-point scale 由 `DILA_SCALE` 寄存器和状态 frame 提供。禁止在协议里默认为“浮点”。
...
| 内部消息 | 所有数据源 | valid/ready + metadata | `msg_stream` | `stream_arbiter` | 是 |
...
每块 V1.0 分配 0x100 B；以后扩展可使用下一页或 32-bit 高地址。
...
| `+0x24` | LPF_PROFILE | RO | V1.0 固定 Butterworth profile ID |
...
| `0x6618` | PROTOCOL_MODE | RW | 0=Modbus RTU，1=ASCII；V1.0 推荐 Modbus |
...
| 地址 | 名称 | R/W | 说明 |
|---:|---|---|---|
| `0x7010` | ARB_MODE | RW | 0=RR；1=priority+RR |
| `0x7014` | TX_FIFO_LEVEL | RO | packetizer 后 FIFO |
| `0x7018` | TX_FIFO_HIGH_WATER | RW | almost full |
| `0x701C` | TOTAL_DROP_COUNT | RO/W1C | 数据丢失总数 |
| `0x7020` | ADC0_FIFO_LEVEL | RO | 诊断 |
| `0x7024` | ADC1_FIFO_LEVEL | RO | 诊断 |
| `0x7028` | DILA0_FIFO_LEVEL | RO | 诊断 |
| `0x702C` | DILA1_FIFO_LEVEL | RO | 诊断 |
| `0x7030` | SENSOR_FIFO_LEVEL | RO | 诊断 |
...
EPSILON2 手册指出高精度同步需要数据协议 + 1PPS。`SYNC_IN` 上升沿作为 V1.0 默认有效沿。边沿经同步器后捕获会引入固定 2~3 sys_clk cycle 延迟；该延迟稳定，可后续标定。不得软件修改 `timestamp_now` 以“对齐 UTC”。
...
`msg_stream`，`source_id=0x0030/0x0031`，`msg_id=DILA_BLOCK`。

### 滤波器约束

V1.0 Butterworth 阶数/系数固定为 compile-time profile。寄存器只暴露 `LPF_PROFILE` 只读 ID，避免上位机误以为系数可任意修改。
...
V1.0 定义为收到响应帧第一个有效字节时的 local tick。
...
3. 校验；
4. 提取 message ID；
5. V1.0 默认**转发完整已校验 raw FDILink frame**；
...
V1.0 使用 GPIO 软件 TTL UART：38400, 8N1，device address 1。

### 协议选择

RD105 同时支持 ASCII 与 Modbus RTU。V1.0 推荐 **Modbus RTU**，因为 CRC 和寄存器访问更适合 FPGA。
...
- 调度共享 I²C；
- 为多个低速 sensor msg FIFO 做 RR；
- sensor online/offline 状态汇总；
- 不解析 USB；
- 不重新打时间戳。

传感器 driver 自己锁存测量 timestamp，sensor_hub 只转发。
...
同级 round-robin。仲裁一旦选择某一 frame，必须直到 `m_last` 才切换，禁止在 frame 中间 interleave。
...
对 `msg_stream` 保存 data/keep/last；SOF metadata 要么一起写入宽 FIFO，要么用 metadata FIFO 与 payload FIFO 严格一一对应。
...
把 I1/Q1/I2/Q2（及可选 magnitude）按照第 4.4 节组成 `DILA_BLOCK`，负责 point_count、first_point_index、cycle_id/timestamp 关联。
...
该模块不属于 V1.0 必须交付，但接口必须与 `dac_ad5791_if` 上层 sample contract 兼容。若后续因 WMS 正弦频率/点数要求切换到 AD3552R：
...
WMS1 -> DAC1 -> laser1                 detector1 -> ADC1 capture FIFO|
  ...
  +--------------------------------------------------->+--> RAW ------+
                                                                   |
PTB/HMP/EPS/BMP/SHT/TFA/RD -> sensor_hub --------------------------+
...
当以下条件全部满足，可以认为 V1.0 下位机具备完整开发闭环：
