...
> 任务定位：负责完整“探测器数字采集 → ADC接口 → 缓冲/CDC → DILA 1f/2f 解调 → 标准 DILA bulk fragment 输出”链路。
...
DILA 输入 ADC 样点，对 WMS 信号执行数字锁相/正交解调，得到 1f/2f I/Q，并经过固定结构低通滤波和抽取后形成标准 `DILA_BLOCK` V2 fragments。
...
# 12. DILA 输出 `bulk_stream` 与分片要求

DILA0 和 DILA1 分别输出独立 `bulk_stream`：
...
- DILA0 source `0x0030`；
- DILA1 source `0x0031`；
- `b_msg_id = 0x1001 (DILA_BLOCK)`；
- `schema_version=2`；
- 默认 `FRAGMENT_POINTS=256`；
- 单 fragment payload 不得超过 8192 B。
...
```text
point 0: int32 H1[0], int32 H2[0]
point 1: int32 H1[1], int32 H2[1]
...
```

即 `format=0`、8 B/point。允许保留调试格式：

- format=1：I1/Q1/I2/Q2；
- format=2：I/Q + MAG1/MAG2。

## 12.2 Fragment header

payload 前 32 B 必须严格遵守主开发指南 V1.1：`schema_version/format/output_rate_hz/total_point_count/fragment_index/fragment_count/first_point_index/fragment_point_count/bytes_per_point/reserved`。

同一 WMS 周期所有 fragment：

- `b_timestamp` 相同；
- `b_cycle_id` 相同；
- `fragment_index` 从 0 连续递增；
- 最后一片允许少于 256 点。

## 12.3 输出缓存

DILA0/DILA1 各自必须拥有独立 bulk FIFO 或 ping-pong fragment buffer：

- 至少能缓存 2 个最大 fragment；
- `b_valid=1 && b_ready=0` 时输出稳定；
- FIFO full 不允许覆盖旧数据；
- 必须设置 ERROR/`DROP_COUNT`；
- 项目负责人在 fragment 完整结束后才会切换 source，因此成员1不需要实现系统级仲裁。
...
| `+0x3C` | `FIFO_LEVEL` | RO | bulk FIFO level |
| `+0x40` | `FRAGMENT_POINTS` | RW shadow | 默认256，合法1..256 |
| `+0x44` | `LAST_FRAGMENT_COUNT` | RO | 最近周期 fragment 数 |
| `+0x48` | `DROP_COUNT` | RO/W1C | fragment/周期丢弃统计 |
...
- [ ] DILA `bulk_stream` fragment 背压 PASS；
- [ ] 双通道并行 PASS；
