...
所有非 ADC 传感器模块完成各自协议后，不直接操作 USB，而统一输出一个标准事件/消息接口。
...
sensor_msg_valid
sensor_msg_ready
sensor_msg_source[7:0]
sensor_msg_type[7:0]
sensor_msg_timestamp[63:0]
sensor_msg_length[15:0]
sensor_msg_data[31:0]
sensor_msg_last
sensor_msg_status[15:0]
```

由 `sensor_hub` 负责轮询/调度并输出到统一 `stream_arbiter`。

这样项目成员只需交付“设备协议 + 标准消息”，无需知道 USB 底层格式。
...
用于 DILA、传感器、状态 → packetizer。

这是 AXI-Stream 风格但不强制引入 AXI IP，成员用纯 Verilog 即可实现。
...
ADC3660 ─→ adc1 ─┬─→ dila1 ────────────┤
                 └─→ adc_cycle_framer1 ─┤
PTB/HMP/EPS/BMP/SHT/TFA/RD105 ─sensor_hub┤
stepper/status/time events ───────────────┤
                                          ▼
                                    stream_arbiter
                                          ▼
                                    data_packetizer
...
- ADC 原始周期：每路至少双 buffer / ping-pong BRAM；
- DILA：小型 FIFO；
- 每个低速传感器：至少 4–16 帧 FIFO；
