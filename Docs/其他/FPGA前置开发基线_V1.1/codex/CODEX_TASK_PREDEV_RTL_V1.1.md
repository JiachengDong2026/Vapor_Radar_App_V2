...
1. `docs/COMMON_INTERFACE_SPEC_V1.1.md`
2. `docs/REGISTER_MAP_FREEZE_V1.1.md`
3. `rtl/common/project_defs.vh`
4. `rtl/common/stream_defs.vh`
5. `rtl/common/register_map.vh`
6. `rtl/common/error_codes.vh`
7. 三成员任务书 V1.3
8. `FPGA下位机Verilog开发指南_V1.1.md`

优先级：本任务提供的 `COMMON_INTERFACE_SPEC_V1.1` 与 `REGISTER_MAP_FREEZE_V1.1` 为成员开工接口基线。若旧文档存在命名/地址冲突，**不要擅自兼容两套定义**；保留 V1.1 基线并在最终报告列出冲突。
...
本任务本身仍只实现 timebase stub、cfg_bus BFM、WMS reference stub 和 clock/reset 仿真源，不提前实现正式 stream arbiter/FIFO。
但所有新增测试代码和公共 header 必须能够编译 V1.1 定义，并明确区分：`sample_stream`、`msg_stream`、`bulk_stream`。
不得把 DILA 正式输出重新定义为 `msg_stream`。
